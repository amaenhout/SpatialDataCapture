function mapcountCity(countryselect, category){
	function getColor(d) {
  ...
  var basemap = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}', {
	...
	if (countryselect == "vienna"){
		...
		var	mymap = new L.map('map-canvas-city',{ zoomControl: true, layers: [basemap]}).setView([46.204391, 6.143158], 13);
	}
	...
	setTimeout(function(){
		...
		// L.control.layers(overlayMaps).addTo(mymap);
		 ...
		 legend.onAdd = function (mymap) {
			...
			// 			 div.innerHTML +=
			// 					 '<i style="background:' + colour_array[i] + '"></i> '+
			// 					 grades[i+1] + (grades[i+1] ? ' - ' + grades[i] + '<br>' : ' to ' + grades[3] + '<br>')}
			// 			else{
			// 					div.innerHTML +=
			// 						'<i style="background:' + colour_array[i] + '"></i> ' + '< ' + grades[i] + ' ' + '<br>';
			// 					 }
